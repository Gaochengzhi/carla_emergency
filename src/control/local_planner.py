...
class LocalPlanner():
    def __init__(self, world, global_waypoints, ego_vehicle, config) -> None:
        ...
        self.controller = VehiclePIDController(
            self.ego_vehicle, self._args_lateral_dict, self._args_longitudinal_dict)
    ...
    def run_step(self, speed):
        ...
        if len(self.local_planner_buffer) < 20:
            try:
                self.local_planner_buffer = self.generate_smooth_trajectory(
                    self.ego_vehicle, wp)
            except:
    ...
    def generate_smooth_trajectory(self, ego, waypoints):
        ...
        for waypoint in waypoints[1:]:
            ...
            distance = compute_distance(
                last_waypoint.transform.location, waypoint.transform.location)
            if distance >= 0.5:
