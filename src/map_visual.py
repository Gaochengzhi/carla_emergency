...
class CenterMap(object):
    def __init__(self,world,map,display,config) -> None:
        self.width = config["screen_main_width"]
        self.height = config["screen_main_height"]
        self.map_image = MapImage(world, map, config["map_precision"], False,True, False)
        self.zoom_level = 1
        self.view_offset_x = 0
        self.view_offset_y = 0
        self.display = display
        self.scaled_map_surface, _ = self.scale_map(self.map_image.big_map_surface, self.zoom_level)
    def redraw(self):
        self.display.fill(COLOR_ALUMINIUM_4)
        self.display.blit(self.scaled_map_surface, (self.view_offset_x, self.view_offset_y))
        pygame.display.flip()

    def scale_map(self,surface, zoom_level):
        original_width, original_height = surface.get_size()
        new_dimensions = (int(original_width * zoom_level), int(original_height * zoom_level))
        return pygame.transform.scale(surface, new_dimensions), new_dimensions
    ...
    def tick(self):
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_PLUS or event.key == pygame.K_EQUALS:  # Zoom in
                    self.zoom_level += 0.1
                    self.scaled_map_surface, _ = self.scale_map(self.scaled_map_surface, self.zoom_level)
        ...
        if keys[pygame.K_UP]:
            self.view_offset_y += 10
        if keys[pygame.K_DOWN]:
            self.view_offset_y -= 10
        if keys[pygame.K_LEFT]:
            self.view_offset_x += 10
...
class MapImage(object):
    ...
    def draw_road_map(self, map_surface, carla_world, carla_map, world_to_pixel, world_to_pixel_width):
        ...
        def draw_arrow(surface, transform, i, color=COLOR_ALUMINIUM_2):
            ...
            for waypoints in set_waypoints:
                ...
                if not waypoint.is_junction:
                    ...
                    for n, wp in enumerate(waypoints):
                        if ((n + 1) % 400) == 0:
                            draw_arrow(map_surface, wp.transform,n )
...
class World(object):
    ...
    def _compute_scale(self, scale_factor):
        ...
        px = (m[0] - self.scale_offset[0]) / float(self.prev_scaled_size)+0.01
        py = (m[1] - self.scale_offset[1]) / float(self.prev_scaled_size)+0.01
...
class InputControl(object):
    ...
    def _parse_keys(self, milliseconds):
        ...
        keys = pygame.key.get_pressed()
        if keys[pygame.K_UP]:
            self.mouse_offset[1]+= 8
        if keys[pygame.K_DOWN]:
            self.mouse_offset[1]-= 8
        if keys[pygame.K_LEFT]:
            self.mouse_offset[0]+= 8
        if keys[pygame.K_RIGHT]:
            self.mouse_offset[0]-= 8
