...
class VehiclePIDControllers():
    def __init__(self, vehicle, args_lateral, args_longitudinal, offset=0, max_throttle=0.55, max_brake=0.8, max_steering=0.5, history_size=10):
    ...
    def _compute_control(self, target_speed, waypoint):
        ...
        # Steering
        control.steer = min(
            max(current_steering, -self.max_steer), self.max_steer)
    ...
    def _smooth_control(self, current_control):
        ...
        avg_control.throttle = (
            avg_control.throttle + current_control.throttle) / (len(self.control_history) + 1)
        avg_control.brake = (
            avg_control.brake + current_control.brake) / (len(self.control_history) + 1)
        avg_control.steer = (
            avg_control.steer + current_control.steer) / (len(self.control_history) + 1)
